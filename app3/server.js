...
app.prepare().then(() => {
  ...
  if (process.env.ASSETPREFIX && process.env.ASSETPREFIX != '') {
    server.use(function(req, res, next) {
      req.url = req.originalUrl.replace(`${process.env.ASSETPREFIX.substr(1)}/_next`, '_next');
      next();
    });
  }
  ...
  if (process.env.ASSETPREFIX && process.env.ASSETPREFIX != '') {
    server.use(function(req, res, next) {
      req.url = req.originalUrl.replace(`${process.env.ASSETPREFIX.substr(1)}/static`, 'static');
      next();
    });
  }
